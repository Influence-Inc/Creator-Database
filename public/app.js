...
(function () {
  ...
  function scoutAccountsPanel() {
    ...
    var rows = (state.scoutUsers || []).map(function (u) {
      // Read-only on purpose: a scout sets their own handle on their sheet.
      // They know it, so it's typed correctly, and the automatic matching
      // actually lands. This column is here so an admin can see at a glance who
      // is connected — not to do the typing for them.
      var ig = u.instagramHandle
        ? '<span class="mono" style="font-size:12px">@' + esc(u.instagramHandle) + '</span>' +
          (u.instagramLinked
            ? '<span class="ig-state ig-ok" title="A message from this account has been received">connected</span>'
            : '<span class="ig-state ig-wait" title="Waiting for their first DM">awaiting DM</span>')
        : '<span class="sheet-muted">not set</span>';
